...
for(let country in countries_data){
	countries_data[country].languages.forEach(language => {
		//si es undefined porque el idioma aún no ha sido encontrado
		if(contLenguajes[language] === undefined){
			//entonces le asigno el valor 0
			contLenguajes[language]=0;
		}
		contLenguajes[language]+=1;//incremento
	});
}
console.log(contLenguajes);
// Convertir el objeto languageCount a un array de pares [idioma, conteo]
const contLanguagesArray = Object.entries(contLenguajes);
//console.log(contLanguagesArray);
// Ordenar el array por el conteo de forma descendente
contLanguagesArray.sort((a,b) => b[1] - a[1]);

const top10Languages = contLanguagesArray.slice(0,11);
...
function sortMostPopulated(countries,n) {
	let mostPopulated= countries.sort((a,b) => b.population - a.population);
	let topNMostPopulated=mostPopulated.slice(0,n+1);
	return topNMostPopulated;
}
console.log(sortMostPopulated(countries_data,10));